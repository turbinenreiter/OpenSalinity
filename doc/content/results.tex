...
A spacial resolution of \unit[10]{mm} would require a time resolution of \unit[0.01]{s} for a velocity of \unitfrac[1]{m}{s}. However, this assumes the size $ l $ of the sensor and the time $ \tau $ a measurement takes to be infinitesimal small, which in reality they are not. While the sensor measures, the flow continues and instead of measuring the conductivity of a certain volume at a certain time, an average is measured. Figure \ref{fig:cv} visualizes this issue.
...
\begin{figure}
	\begin{center}
		...
		\caption[The control volumes in blue have the same size as the sensor (black rectangle).]{The control volumes in blue have the same size as the sensor (black rectangle). At $t=0$ the measurement starts and the first control volume is directly over the sensor. When the measurement ends at $t=\tau$ the control volumes have moved. The sensor is covered by a mix of the two volumes.}
		\label{fig:cv}
	\end{center}
\end{figure}
...
To test if the time resolution matches this, the measurement time itself can be timed. First, the difference between two adjacent times in the data set in Listing \ref{lst:data} is calculated. The mean of the differences through the complete data set is \unit[748]{$\mu$s}. This time includes the \unit[520]{$\mu$s} it takes to switch from one electrode pair to the next. The resulting net measurement time is \unit[228]{$\mu$s}, the resulting ratio $r$ is $43.9$. This means that the measurement is more than ten times faster than required, showing that the time resolution easily matches the spacial resolution. The measured volume is only \unit[2.28]{\%} bigger than the control volume.
...
While the requirement for sensitivity is clearly met, the range is just barely acceptable. For validation, the used range has to be adjusted and will show if it is sufficient.
...
\begin{figure}[H]
	\begin{center}
		\input{images/residenceTime}
		\caption[Step change of the salinity of the feed and the expected response [by Timm Severin].]{Step change of the salinity of the feed (\drawline[red!80!white]) and the expected response (\drawline[blue!80!white, dashed]) [by Timm Severin].}
		\label{fig:rest}
	\end{center}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		\input{images/log080716_2_7_scatter}
		%\includegraphics[width=0.8\textwidth]{images/log080716_2_7_scatter.pdf} 
		\caption[Filter design]{Filter design - The scatter plot shows the raw data (\drawline[cyan]), filtered for local maxima (\drawline[blue!80!white]) and smoothed with a Savitzky-Golay filter (\drawline[red]).}
	...
	\tikzset{external/export next=false}
		\begin{tikzpicture}[scale=0.9]
			\begin{scope}[local bounding box=unadj]
				\input{images/log080716_2_unadj}
				\node[very thick] at (0,-1) {(a)};
			\end{scope}
			\begin{scope}[xshift=0.55\textwidth]
				\input{images/log080716_2_adj}
...
Another interesting data point is the the behavior when the water flow is first switched on. Figure \ref{fig:test2_on} shows the signals during this phase. Signal four in the inlet basin rises first, signal eight rises about $\diff t =  \unit[4]{s}$ later. This is the time it takes the empty basin to fill up completely, only after which the water flows over the weir. At a flow rate of \unitfrac[1.6]{l}{s}, this suggests a volume of the inlet basin of around \unit[6.4]{l}. The real volume calculated according to the schematic is about \unit[5.8]{l}.

\begin{figure}[H]
	\begin{center}
		\input{images/log080716_2_on}
...
Again the impulse method was used but with an increased flow rate of \unitfrac[2]{l}{s} and a lower salinity of \unit[6.7]{\%}. The time $t_{cycle}$ from first to second peak is now about \unit[41]{s}, as expected due to the higher flow rate. The voltages are also a little lower because of the lower salinity.

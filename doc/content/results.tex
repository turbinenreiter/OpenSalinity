...
\begin{figure}[H]
	\begin{center}
		...
		\caption[The two control volumes in light and dark blue have the same size as the sensor shown as black rectangle]{The two control volumes in light and dark blue have the same size as the sensor shown as black rectangle. At $t=0$ the measurement starts and the first control volume is directly over the sensor. When the measurement ends at $t=\tau$, the control volumes have moved. The sensor is covered by a mix of the two volumes.}
		\label{fig:cv}
	\end{center}
\end{figure}

Factors $ n $ \eqref{eq:resn} and $ k $ \eqref{eq:resk} are introduced to accommodate for this problem, describing the ratio of the resolutions to the actual sizes.

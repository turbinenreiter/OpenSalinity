...
Figure \ref{fig:isys} shows the device developed in this thesis. The following section describes the design of the sensor system and all components involved.
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
		\caption{The developed sensor system. The microcontroler and USB connection (\drawline[red,ultra thick]) control all parts and connect to the host PC. The Matrix Switches (\drawline[red!50!yellow,ultra thick]) connect the MinieC (\drawline[blue,ultra thick]) to the sensor strip (\drawline[blue!75!white,ultra thick]) containing the sensors (\drawline[blue!50!white,ultra thick]) via the connectors (\drawline[red!25!yellow,ultra thick]).}
		\label{fig:isys}
	\end{center}
\end{figure}
...
\end{tikzpicture}
		...
		\caption{System Design: the yellow area marks the user facing side of the system, the green area is the hardware that is mounted on the carrier board and the blue area marks the sensors deployed in the water stream. The rectangle marks the part of the system that is repeated $n$ times to get to the needed number of sensors.}
		\label{fig:sys}
	\end{center}
\end{figure}
